...
class PruneLinear(nn.Module):
    ...
    def prune_by_percentage(self, q):
        """
        Pruning the weight paramters by threshold.:param q: pruning percentile. 'q' percent of the least 
        significant weight parameters will be pruned.
        """
        np_weight = self.linear.weight.data.cpu().numpy()
        flattened_weights = np.abs(np_weight.flatten()) # taking abs value & flatten
        N = flattened_weights.shape[0]
        # Generate the pruning threshold according to 'prune by percentage'. (Your code: 1 Line) (ascending)
        # anything less than thres will be pruned; anything >= thres will be kept
        thresh = np.sort(flattened_weights, axis=None)[int(q * N / 100)]
        # Generate a mask to determine which weights should be pruned (Your code: <=3 Lines)
        # mask = 1 keep; mask = 0 throw
        ...
        self.linear.weight.data = torch.from_numpy(np_weight).to(device)
        # Compute sparsity (Your code: 1 Line) # change
        self.sparsity = 1.0 - np.sum(self.mask,axis = None) / flattened_weights.shape[0]
...
class PrunedConv(nn.Module):
    ...
    def prune_by_percentage(self, q):
        ...
        self.conv.weight.data = torch.from_numpy(np_weight).to(device)
